...
export const Contactos = () => {
  ...
  const handlerDeleteContact = async (contacto) => {
    try {
      await contactosAgendas.deleteContact(contacto);
  ...
  const handlerEditar = async (contacto) => {
    try {
      navigate(`/addcontact/${contacto.id}`)
      const responseContactos = await contactosAgendas.getAllContactos(usuarioActual);

      const contacteid = responseContactos.contacts.filter((contactoid) => {
        return contacto.id === contactoid.id
      })
      dispatch({
        type: "get_contactos_userid",
        payload: { contacts_elegido: contacteid[0] }
      });
